...
const systemInstruction = `You are a social media feed generator. Your task is to create a list of realistic social media posts based on a set of keywords. These posts must be from people LOOKING FOR an English language tutor or English lessons for themselves or their children.
...
const generateUserPrompt = (keywords: string[], platforms: string[]): string => `
The user is searching for posts from the following platforms: [${platforms.join(', ')}] with the following keywords: [${keywords.join(', ')}].

Generate 5 to 10 posts. For each post, provide the following information in the specified JSON format.
- platform: Must be one of the requested platforms: '${platforms.join("', '")}'.
...
export default async function handler(
  ...
  try {
    ...
    const userPrompt = generateUserPrompt(keywords, platforms);
